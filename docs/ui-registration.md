...
- 右ペイン: タグ管理画面 (`/tags`) でのみ表示し、選択中エンティティの詳細・編集を担当する
...
タグ管理画面では、ScanGroup の新規作成や削除は引き続きドライバ UI 側を正本とするが、
**既存 ScanGroup の周期（`scan_rate_ms`）変更のみ** は本体アプリ右ペインから実行できるものとする。

対象操作は以下の 2 種類とする。

- **グループ単位変更**: 選択中の ScanGroup 1 件の `scan_rate_ms` を更新する
- **接続先単位一括変更**: 選択中の接続先配下に属する全 ScanGroup の `scan_rate_ms` を同一値で更新する

制約:

- 変更対象は `scan_rate_ms` のみとし、`scan_group.id` / `table` / `timestamp_column` / `node` などの構造項目は本体 UI から変更しない
- ScanGroup の追加・削除・ID 変更は従来どおりドライバ UI で行う
- 更新キーは `driver_id + scan_group_id` を正本とし、`scan_group_id` 単独では扱わない

### 右ペインの周期変更導線

`/tags` の右ペインでは、選択対象に応じて以下の編集導線を提供する。

- **接続先選択時**
  - 接続先配下の ScanGroup 件数を表示する
  - 一括変更用の周期入力欄（ms）を表示する
  - 「この接続先の全 ScanGroup に適用」ボタンを表示する
- **ScanGroup 選択時**
  - 現在の周期（`scan_rate_ms`）を表示する
  - 単体変更用の周期入力欄（ms）を表示する
  - 「この ScanGroup に適用」ボタンを表示する

UI ルール:

- 入力欄は数値入力とし、最小値は 100ms とする
- 実行中は対象ボタンを無効化し、二重送信を防止する
- 成功時はタグ管理画面下部のメッセージ領域に結果を表示する
- 失敗時は既存の通知導線でエラーを表示する
- ドライバ UI 起動中の編集ロック方針がある場合はそれに従い、必要なら本体側操作を抑止する
...
- 「削除」: 確認ダイアログ後、本体側で削除
- 「閉じる」: 詳細表示をクリア
- 「この ScanGroup に適用」: 選択中 ScanGroup の周期のみ更新
- 「この接続先の全 ScanGroup に適用」: 選択中接続先配下の全 ScanGroup の周期を一括更新
...
JoyWatcher 登録 UI は、vendor DLL を x86 bridge 経由で利用しながら、
**接続先単位でタグ選択・nativeTagId 解決・型確認・保存** を行う専用 UI とする。

JoyWatcher は PostgreSQL や SLMP と異なり、登録時点ではタグ型が確定しない。
そのため登録 UI では、

- タグ選択は `TagSel2`
- タグ名 → `nativeTagId` 解決は `JWGetTagIDS2`
- 実型確認は `JWRead`
...
JoyWatcher 登録 UI の標準フローは以下とする。

```text
接続先設定
  → タグ登録 (TagSel2)
  → タグ名解析
  → nativeTagId 解決 (JWGetTagIDS2)
  → グループ別に一覧化
  → 型確認 (JWRead)
  → 確認・保存
```
...
- `TagSel2` は vendor 標準ダイアログを開き、選択済みタグ一覧を返す
- 選択タグは `接続先ID$グループ.タグ名$VALUE` 形式の文字列として返る
- UI はこの文字列を解析し、`connection` / `group` / `tag` を抽出する
- 保存時は `driverSpec.tagPath` と `driverSpec.nativeTagId` を必須とする

### 接続先情報 UI

JoyWatcher 登録 UI の step 1 先頭には、**SelTag2 の結果から判定した接続先情報を表示する領域** を置く。

前提として、JoyWatcher の接続先判定は **ユーザー手入力ではなく SelTag2 の結果** を正本とする。
したがって、接続先に関するフォーム入力は置かず、UI は表示専用とする。
...
- `driverId` の正本は **SelTag2 の結果から得られる接続先 ID** を優先する
- ユーザーが endpoint / user_id / password を入力する導線は持たない
- launch context に接続設定が存在しても、それは bridge 実行用の内部値として扱い、UI の主入力にはしない
...
接続先情報
  ↓
TagSel2 / JWRead / 確認・保存 のアクション列
  ↓
左: グループ一覧 / 右: 選択グループ詳細
```

つまり、操作ボタンよりも先に「SelTag2 の結果としてどの接続先を扱っているか」を視認できるようにする。

#### 表示ルール

- SelTag2 実行前は「未取込」または「接続先未確定」と分かる表示にする
- SelTag2 実行後は、結果から得られた接続先 ID を表示する
- 複数の接続先 ID が混在した場合は、保存前に警告する
- 既に取り込んだタグがすべて同一接続先由来であることを UI 上で確認できるようにする

#### 接続先IDとの関係

JoyWatcher では、接続先 ID は SelTag2 が返す値を正本として扱う。

- `driver.id` は最終的に **SelTag2 結果の接続先 ID** を優先して決定する
- グループの `node` も同じ接続先 ID を保持する
- もし SelTag2 結果中に複数の接続先 ID が含まれる場合、保存前レビューで警告する

#### 確認・保存時の扱い

保存時の接続先識別は `driver.id` を正本とし、これは SelTag2 の結果から確定した値を使う。

UI は接続先情報を表示するが、ユーザー入力は受け付けない。
内部的に bridge 実行用の接続設定を保持する場合でも、それは UI の主操作対象ではない。

### タグパス形式

`TagSel2` で返るタグパスは以下の形式を前提とする。

```text
<connection>$<group>.<tag>$VALUE
```

例:

- `LOCAL$TEST_OPB_1MIN.NONAME5$VALUE`
- `LOCAL$REPORT.NONAME4$VALUE`
- `LOCAL$BTE1B.B0408$VALUE`

UI 側はこの形式を前提に、以下のように扱う。
...
JoyWatcher 登録 UI は PostgreSQL 登録 UI に近い見た目を保ちつつ、
JoyWatcher 特有のタグ選択・型確認操作を上部アクションに寄せる。

#### 上部アクション

- `タグ登録 (TagSel2)`
  - vendor のタグ選択ダイアログを開く
  - 選択したタグを UI へ取り込む
- `型確認 (JWRead)`
  - 取り込み済みタグの `nativeTagId` を使って `JWRead` する
  - 型確認結果を一覧へ反映する
- `確認・保存へ進む`
  - 現在の接続先 / グループ / タグ状態をレビュー画面へ送る
...
JoyWatcher ではタグ選択結果からグループが決まるため、
PostgreSQL のようにテーブルを先に選ぶのではなく、**TagSel2 の結果からグループを自動生成** する。
...
`タグID` は `JWGetTagIDS2` で解決した `nativeTagId` を表示する。

### 型確認 (`JWRead`) の仕様

JoyWatcher では、登録時にタグ型を完全には確定できない。
したがって、**登録 UI の型確認機能は `JWRead` の結果を正本とする。**

#### 重要ルール

- 保存前に表示している `dataType` は暫定値である
- 実運用で信用すべき型は **`JWRead` の `TCOM_DATA1.dtype` に基づく型** である
- UI は `dtype` の生値ではなく、現行 bridge の集約結果を表示する

現行 bridge の型マッピングは以下。

| JoyWatcher `dtype` | UI 表示型 |
| ---: | --- |
| 0 | `number` |
| 1 | `number` |
| 2 | `number` |
...
- 数値系は `WORD` / `DWORD` / `SINGLE` / `DOUBLE` / `UWORD` / `ULONG` を区別せず `number` へ集約する
- 必要になれば将来 `dtype` または `numericKind` を UI へ返す拡張を検討する

#### 型確認ボタンの挙動

- 実行中は再入禁止とし、ボタンを無効化する
- 実行中は `型確認中...` のような状態表示を出す
- 型が未確認のタグは `未設定` 表示とする
- 確認後は型バッジなど、一覧中で見分けやすい表示にする

### 保存データの扱い

JoyWatcher 登録 UI は、保存時に少なくとも以下を本体へ返す。

- 接続先定義
- グループ一覧
- タグ一覧
- 各タグの `driverSpec.tagPath`
- 各タグの `driverSpec.nativeTagId`

補足:

- `nativeTagId` は本体タグ ID とは別物として扱う
- 本体 / runtime は JoyWatcher 値取得時に `nativeTagId` を使用する
- UI 上の暫定 `dataType` は保持してもよいが、JoyWatcher 実型の正本ではない

### UX 上の注意点

- `TagSel2` で一括選択したタグは、グループ別に自動でまとめて表示する
- 追加選択時は既存グループへ自然にマージする
- 型未確認でも保存は可能だが、UI では「未確認」であることを明示する
- 通信失敗時は「タグ選択」「ID解決」「型確認」のどの段階で失敗したか分かる文言を出す
- bridge や DLL の問題で UI が固まらないよう、重い呼び出しはバックグラウンド実行し、待機状態を可視化する

### 現時点で UI 仕様として確定していること

- JoyWatcher 登録 UI は PostgreSQL 風のレイアウトを採用するが、フローは JoyWatcher 専用とする
- タグ選択の正規ルートは `TagSel2`
- `nativeTagId` 解決の正規ルートは `JWGetTagIDS2`
- 型確認の正規ルートは `JWRead`
- 型確認結果の正本は `JWRead` 由来の型であり、保存前の `dataType` ではない
