...
pub(crate) async fn sync_driver_runtime(
    ...
    let should_keep_driver_running =
        runtime_status.drivers_running || runtime_status.publishers_running;
    ...
    if was_running {
        info!(
            "Driver config changed while runtime active; restarting driver: {}",
            config.id
        );
